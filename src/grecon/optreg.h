...
extern void opt_reg_configure(int N, const long img_dims[__VLA(N)], struct opt_reg_s* ropts, const struct operator_p_s* prox_ops[NUM_REGS], const struct linop_s* trafos[NUM_REGS], const long llr_blk[__VLA(N)], unsigned int shift_mode, const char* wtype_str, _Bool use_gpu);
